  ...
  return (
    <>
      ...
      <details className="mb-6 rounded-mi bg-mi-superficie-elevada p-4 shadow-suave">
        <summary className="cursor-pointer font-corpo text-sm text-mi-marrom">＋ Novo recorrente</summary>
        <form action={adminCreateRecurring} className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <label className="block">
            <span className="mb-1 block text-xs text-mi-texto/60">Dia de vencimento (1–28)</span>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {recorrentes.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-mi-texto/55">Nenhum custo recorrente cadastrado.</td></tr>
            )}
            {recorrentes.map((r) => (
              <tr key={r.id} className="border-b border-mi-cinza/60">
                ...
                <td className="px-4 py-3">
                  {r.active ? (
                    ...
                    <span className="text-xs text-mi-texto/50">inativo</span>
                  )}
                </td>
