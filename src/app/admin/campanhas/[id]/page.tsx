...
function Card({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-mi bg-mi-branco p-4 shadow-suave">
      <p className="text-xs uppercase tracking-wide text-mi-texto/55">{label}</p>
  ...
  return (
    <>
      ...
      <div className="mb-4 rounded-mi bg-mi-branco p-4 shadow-suave">
        <p className="text-sm text-mi-texto/70">Mensagem</p>
      ...
      <div className="flex flex-wrap gap-3">
        <form action={dispararTesteAction.bind(null, camp.id)}>
          <button className="rounded-mi border border-mi-marrom px-4 py-2 text-sm text-mi-marrom hover:bg-mi-marrom hover:text-white">
        ...
        {m.pendentes > 0 && (
          <form action={aprovarPendentesAction.bind(null, camp.id)}>
            <button className="rounded-mi bg-emerald-600 px-4 py-2 text-sm text-white">
