...
export default async function AdminLayout({
  ...
  // banco se a senha foi trocada desde então (roda no Edge, sem Prisma). A
  // conferência de verdade é `exigirSessaoDoPainel()`: aqui, no carregamento
  // completo, e no topo de CADA página — porque o App Router não roda o
  // layout de novo em navegação interna (<Link>), só a página. Sem isso a
  // sessão derrubada em outro aparelho seguia navegando pelo painel.
  ...
  // pública: a guarda de cada página/action continua protegendo, e é melhor
  // degradar para a tela antiga do que trancar a Mi para fora.
  const publica =
    !rota || PUBLICAS.some((p) => rota === p || rota.startsWith(`${p}/`));

  if (!publica) await exigirSessaoDoPainel();
