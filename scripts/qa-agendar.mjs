...
if (/miozorio\.com\.br/i.test(BASE)) {
  console.error("Este roteiro CRIA E CONFIRMA reservas. Não rode contra produção.");
  process.exit(2);
}
...
async function servicoSimples(api) {
  ...
  const s = services.find((x) => !x.variantes?.length && !x.isCourse && x.priceCents > 0);
...
async function primeiroLivre(api, serviceId) {
  for (const date of DIAS) {
    const r = await api.get(`${BASE}/api/availability?serviceId=${serviceId}&date=${date}`);
    const { slots } = await r.json();
    if (slots?.length) return { date, time: slots[0] };
  }
  throw new Error("acabaram os horários livres — limpe as reservas do banco de QA");
}
...
async function criar(api, serviceId, telefone, headers = {}) {
  ...
  return api.post(`${BASE}/api/bookings`, {
    headers,
    data: {
      serviceId, date, time, location: "studio",
...
async function escolherDiaEHora(page) {
  ...
  for (let i = 0; i < (await dias.count()); i++) {
    ...
    const horas = page.locator("main button").filter({ hasText: /^\d\d:\d\d$/ });
    if (await horas.count()) return void (await horas.first().click());
    await page.locator("main button").filter({ hasText: "‹ voltar" }).first().click();
...
await pA.getByText(servico.name, { exact: true }).first().click({ timeout: 20000 });
await escolherDiaEHora(pA);
await preencher(pA, "Cliente QA", "21999990001");
await pA.getByRole("button", { name: "Confirmar meu horário" }).waitFor({ timeout: 20000 });

const posse = (await ctxA.cookies()).filter((c) => c.name.startsWith("mi_posse_"));
ok(posse.length === 1, `comprovante emitido na criação (${posse.length} cookie)`);
const c0 = posse[0];
...
ok(!!idA && !c0.value.includes(idA), "não carrega o id nem PII — só prazo e assinatura");

await pA.getByRole("button", { name: "Confirmar meu horário" }).click();
await pA.waitForTimeout(2500);
ok(/Agendamento confirmado/i.test(await pA.locator("main").innerText()), "tela de sucesso apareceu");
ok(
  (await ctxA.cookies()).filter((c) => c.name.startsWith("mi_posse_")).length === 0,
...
const rE = await ctxEstranho.request.post(`${BASE}/api/bookings/${idB}/confirm`);
ok(rE.status() === 403, `estranho recebe 403 (recebeu ${rE.status()})`);
ok((await rE.json()).code === "sem_posse", "código sem_posse");
const est = await (await ctxEstranho.request.get(`${BASE}/api/bookings/${idB}`)).json();
...
ok(rD.status() === 200, `a dona, no mesmo navegador, confirma normal (${rD.status()})`);
...
ok((await criar(ctxC.request, servico.id, "21977770001")).status() === 201, "outro IP não é afetado");
...
await pC.getByText(servico.name, { exact: true }).first().click({ timeout: 20000 });
...
ok(!/Tenta de novo\?/.test(txt), "NÃO mostra o genérico que mandava repetir na hora");
...
ok(
  (await ap2.request.post(`${BASE}/api/bookings/${idD}/confirm`)).status() === 403,
...
ok((await ap2.cookies()).some((c) => c.name === "mi_clube"), "entrou no Clube neste aparelho");
ok(
  (await ap2.request.post(`${BASE}/api/bookings/${idD}/confirm`)).status() === 200,
  "a dona logada confirma de outro aparelho",
);

const ap3 = await b.newContext();
const idAlheio = (await (await criar(ap3.request, servico.id, "21944440001")).json()).id;
ok(
  (await ap2.request.post(`${BASE}/api/bookings/${idAlheio}/confirm`)).status() === 403,
